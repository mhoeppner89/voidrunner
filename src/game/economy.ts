import { COMMODITIES, DOCK_LOCATION_IDS, LOCATIONS, commodityIds } from './data';
import { clamp, randomBetween, seededRandom } from './random';
...
export const createInitialMarket = (seed: number): PersistentWorldState['market'] => {
  const market = {} as PersistentWorldState['market'];
  for (const locationId of DOCK_LOCATION_IDS) {
    market[locationId] = {} as Record<CommodityId, MarketItemState>;
...
export const refreshAllPrices = (
  market: PersistentWorldState['market'],
  seed: number,
  economyClock: number,
): void => {
  for (const locationId of DOCK_LOCATION_IDS) {
...
export const tickEconomy = (world: PersistentWorldState, seconds: number): void => {
...
export const cargoMass = (player: PlayerState): number => {
...
export const cargoCapacity = (player: PlayerState): number => getEffectiveShipStats(player).cargo;
export const cargoFree = (player: PlayerState): number => Math.max(0, cargoCapacity(player) - cargoMass(player));
...
export const buyCommodity = (
  ...
  if (quantity <= 0) {
    if (item.supply <= 0) return { ok: false, message: 'Market stock exhausted.', quantity: 0, total: 0 };
    if (affordable <= 0) return { ok: false, message: 'Insufficient credits.', quantity: 0, total: 0 };
...
export const sellCommodity = (
  ...
  if (quantity <= 0) return { ok: false, message: 'No matching cargo in the hold.', quantity: 0, total: 0 };
  const item = save.world.market[locationId][commodityId];
