...
export const randomBetween = (rng: () => number, min: number, max: number): number => min + (max - min) * rng();
export const randomInt = (rng: () => number, min: number, maxInclusive: number): number =>
  Math.floor(randomBetween(rng, min, maxInclusive + 1));
export const pick = <T>(rng: () => number, values: readonly T[]): T => values[Math.min(values.length - 1, Math.floor(rng() * values.length))]!;
...
export const proceduralPersonName = (rng: () => number): string => `${pick(rng, GIVEN_NAMES)} ${pick(rng, SURNAMES)}`;
export const proceduralCallsign = (rng: () => number): string => `${proceduralPersonName(rng)} “${pick(rng, CALLSIGNS)}”`;

export const formatCredits = (value: number): string => `${Math.max(0, Math.floor(value)).toLocaleString('en-US')} cr`;
