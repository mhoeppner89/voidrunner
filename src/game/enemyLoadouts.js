...
export function createEnemyLoadout(ship, index) {
    ...
    const preferred={pulse:235,'pulse-mk2':250,beam:240,gauss:510,mortar:300,ion:85,ripper:85};
    const range=Math.min(weaponRange(main),preferred[main.id]??240);
    const role=range>=300?'gunship':range<100?'interceptor':'fighter';
    const profile={role,range,projectileSpeed:main.speed,life:main.life,interval:main.cooldown,
