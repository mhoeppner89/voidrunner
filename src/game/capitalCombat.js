...
export const FRIGATE_GUN=Object.freeze({id:'frigate-battery',kind:'laser',speed:360,range:720,life:2,damageFlat:18,energyCost:8});
export const FRIGATE_BOSS_GUN=Object.freeze({...FRIGATE_GUN,speed:520,range:600,life:600/520,damageFlat:36,energyCost:12});
...
export function equipFrigate(ship,boss=false){
 ...
 if(boss){ship.maxShield=650;ship.shield=650;ship.maxHull=1400;ship.hull=1400;ship.shieldRegen=0;ship.turnRate=.24;}
}
...
export function updateFrigateBatteries(session,ship,dt,targetPosition,targetVelocity){
 ...
 const weapon=ship.capitalBoss?FRIGATE_BOSS_GUN:FRIGATE_GUN;
 let charging=false,firing=false;
 let cycle;
 if(ship.capitalBoss){
  ...
  if(!visible&&cycle.phase!=='RECOVERING'){cycle.phase='RECOVERING';cycle.until=now+5.5;}
  if(now>=cycle.until){
   if(cycle.phase==='RECOVERING'&&visible){cycle.phase='CHARGING';cycle.until=now+2.5;cycle.serial++;}
   else if(cycle.phase==='CHARGING'){cycle.phase='SALVO';cycle.until=now+1.3;}
   else if(cycle.phase==='SALVO'){cycle.phase='RECOVERING';cycle.until=now+5.5;}
  }
 ...
 for(let i=0;i<4;i++){
  ...
  if(target){
   ...
   if(cycle&&(cycle.phase==='SALVO'||cycle.phase==='CHARGING'&&cycle.until-now<=.6)&&gun.lockSerial!==cycle.serial){
    gun.lockSerial=cycle.serial;gun.lockedAt=now;gun.lockedTarget??=new THREE.Vector3();gun.lockedVelocity??=new THREE.Vector3();
    gun.lockedTarget.copy(s.target);gun.lockedVelocity.copy(targetVelocity?.isVector3?targetVelocity:s.velocity.fromArray(targetVelocity??[0,0,0]));gun.rounds=3;gun.fireAt=cycle.phase==='CHARGING'?cycle.until:now;
   }
   ...
   const error=s.position.distanceTo(s.target)*(ship.capitalBoss?.003:.018);
   ...
   if(solution){s.turn.setFromUnitVectors(gun.direction,s.direction);s.turn.slerp(newIdentity,1-Math.min(1,(ship.capitalBoss?1.5:.7)*dt/Math.max(.00001,gun.direction.angleTo(s.direction))));gun.direction.applyQuaternion(s.turn).normalize();}
  }
  ...
  if(cycle){
   if(cycle.phase==='CHARGING'){
    if(solution)session.renderer.showCapitalCharge?.(`${ship.id}-${i}`,s.position,Math.max(0,1-(cycle.until-now)/2.5),ship);
    continue;
   }
   if(cycle.phase!=='SALVO'||!solution||gun.rounds<=0)continue;
  }else if(!solution){gun.chargeAt=0;gun.rounds=0;continue;}
  if(now<gun.fireAt||gun.direction.dot(s.direction)<.998)continue;
  if(!cycle&&!gun.chargeAt){gun.chargeAt=now+(ship.capitalBoss?1.6:1.1)+i*.18;gun.rounds=3;}
  if(!cycle&&now<gun.chargeAt){
   charging=true;
   // Steady amber charge glow, distinct from the short muzzle flash.
   session.renderer.showCapitalCharge?.(`${ship.id}-${i}`,s.position,Math.min(1,1-(gun.chargeAt-now)/1.7),ship);continue;
  }
  ...
  firing=true;gun.rounds--;gun.fireAt=now+(gun.rounds?(ship.capitalBoss?.42:.55):5)*factor;
  if(!gun.rounds)gun.chargeAt=0;
 }
 if(!cycle)ship.capitalAttack=charging?'CHARGING':firing?'FIRING':ship.capitalRuntime.mounts.some(g=>g.rounds>0)?'SALVO':'RECOVERING';
}
...
export function updateFrigateAttack(session,ship,targetPosition,targetVelocity,dt){
 ...
 s.direction.applyAxisAngle(worldUp,s.attackSide*(ship.capitalBoss?1.35-.35*approach:.85-.65*approach));
