import { EQUIPMENT, SHIPS } from './data';
import type { EquipmentId, PlayerState, ShipDefinition } from './types';

export interface EffectiveShipStats extends ShipDefinition {
  scanRange: number;
  miningRate: number;
  salvageRate: number;
  salvageRange: number;
...
export const getEffectiveShipStats = (player: Pick<PlayerState, 'shipId' | 'equipment'>): EffectiveShipStats => {
  const base = SHIPS[player.shipId]!;
  const has = (id: EquipmentId) => player.equipment.includes(id);
  return {
    ...base,
    ...
    scanRange: has('radar-mk2') ? 260 : 165,
...
export const repairCost = (player: PlayerState): number => {
...
export const refillCost = (player: PlayerState): number => {
  const stats = getEffectiveShipStats(player);
...
export const equipmentUnlocked = (player: PlayerState, equipmentId: EquipmentId): boolean => {
  const equipment = EQUIPMENT[equipmentId]!;
  if (!equipment.requiredGuild) return true;
