...
obj=next(o for o in bpy.context.selected_objects if o.type=='MESH');obj.name='Lancer_Refined_Paint_Preview'
mesh=obj.data
...
def paint(p,source):
    x,y,z=p.T
    lum=source[:,:3]@np.array([.2126,.7152,.0722])
    red=(source[:,0]>source[:,1]*1.45)&(source[:,0]>source[:,2]*1.4)&(source[:,0]>.07)
    white=lum>.23
    grey=(lum>.085)&~white
    col=np.broadcast_to(np.array([.035,.045,.054]),(len(p),3)).copy()
    def smooth(a,b,v):
        t=np.clip((v-a)/(b-a),0,1);return t*t*(3-2*t)
    grey_weight=smooth(.065,.11,lum)
    white_weight=smooth(.17,.25,lum)
    col=col*(1-grey_weight[:,None])+np.array([.20,.225,.235])*grey_weight[:,None]
    col=col*(1-white_weight[:,None])+np.array([.63,.65,.64])*white_weight[:,None]
    col[red]=[.44,.028,.018]
    metal=np.where(white|red,.09,.66);rough=np.where(white|red,.54,.42)
